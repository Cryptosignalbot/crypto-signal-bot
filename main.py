    ...
    for email, info in list(users.items()):
        ...
        for stype, sub in list(subs.items()):
            ...
            if secs <= 300 and not sub.get("avisado", False):
                text = (
                    f"⏳ Tu suscripción {TYPE_LABELS[stype]} expira en 24 Horas. Renueva tu suscripción."
                    if lang=="ES"
                    else
                    f"⏳ Your {TYPE_LABELS[stype]} subscription expires in 24 hours. Renew now."
                )
                requests.post(
                    f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",
                    json={"chat_id": cid, "text": text, "reply_markup": {"inline_keyboard":[[{"text":"🔄 Renovar","callback_data":f"renovar_menu|{stype}" }]]}},
                    timeout=10
                )
            ...
            if secs <= 0:
                ...
                requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/kickChatMember", json={"chat_id": group_id, "user_id": cid}, timeout=10)
                requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/unbanChatMember", json={"chat_id": group_id, "user_id": cid}, timeout=10)
                subs.pop(stype)
                modified = True
